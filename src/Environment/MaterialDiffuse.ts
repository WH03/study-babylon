...
export default class BasicScene {
  ...
  CreateGround(): void {
    ...
    const texture = new Texture("/textures/grass.jpg");
    texture.uScale = 10; //纹理的横向重复次数
    texture.vScale = 10; //纹理的纵向重复次数
  ...
  addShadow(mesh: Mesh, light: DirectionalLight) {
    ...
    shadowGenerator.useBlurExponentialShadowMap = true; //使用指数模糊阴影贴图
    shadowGenerator.useKernelBlur = true; //使用核模糊
    shadowGenerator.blurKernel = 64; //模糊核的大小
    shadowGenerator.blurScale = 4; //模糊的缩放比例
    shadowGenerator.depthScale = 0; //模糊盒子的偏移量
  }
