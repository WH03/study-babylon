...
export default class BasicScene {
  ...
  CreateScene(canvas: HTMLCanvasElement): Scene {
    ...
    const freeCamera = new FreeCamera("freeCamera", new Vector3(0, 3, -15)); //创建相机
  ...
  CreatePerson() {
    ...
    body.parent = base;
    head.position.y = 0.5 * (headDiameter + bodyDiameter);
    base.position.y = 0.5 * bodyDiameter - 1;

    this.VisualizeCollisionBoxes(base, headDiameter, bodyDiameter);
  }
  ...
  VisualizeCollisionBoxes(
    ...
    for (let i = -Math.PI / 2; i < Math.PI / 2; i += Math.PI / 36) {
      points.push(new Vector3(0, y * Math.sin(i) + offsetY, x * Math.cos(i)));
    }
